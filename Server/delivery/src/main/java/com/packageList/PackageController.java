...
public class PackageController {
	...
	public Package getPackage(String packageID) {
		JSONObject packageJSObj = new com.database.PackageDatabase().getPackage(packageID);
		if (packageJSObj == null)
			return null;
		
		Package result = new Package();
		result.setAddress((String) packageJSObj.get("addressDelivery"));

		com.database.Date dateD = new Date();
		JSONObject jsonDateD = (JSONObject) packageJSObj.get("dayDelivery");
		...
		JSONObject jsonDateR = (JSONObject) packageJSObj.get("dayReceive");
		...
		Long cost = (long) packageJSObj.get("cost");
		result.setPrice(cost.intValue());

		String status = (String) packageJSObj.get("status");
